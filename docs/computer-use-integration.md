# Computer use & browser use in MultiBot

Decision doc, 2026-08-12. How bots in MultiBot get local computer use and
...
│     one TCC prompt, named MultiBot              │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is MultiBot's, users see ONE
   prompt named MultiBot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `MultiBot.app/Contents/Resources/cua-driver`,
