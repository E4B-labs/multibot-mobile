        ...
        for (;;) {
            ...
            while ((sep = buf.indexOf("\n\n")) !== -1) {
                ...
                if (!name || !data)
                    continue;
                try {
                    yield { name, payload: JSON.parse(data) };
                }
                catch {
...
export const SlafyDriver = {
    ...
    async create(input) {
        ...
        const ensuredBots = new Set();
        const engineBotId = (threadId) => engineBotIdFor(threadId, config.botPrefix);
        const emit = (event) => {
        ...
        const syncConnectors = async (baseUrl) => {
            ...
            const installed = res.ok ? (await res.json()) : [];
            for (const plugin of installed) {
                const name = String(plugin?.name ?? "");
                if (!name.startsWith(ENGINE_PLUGIN_PREFIX) || names.has(name))
                    continue;
            ...
            for (const connector of wanted) {
                ...
                if (!install.ok) {
                    throw new Error(`engine POST /api/plugins/install (${connector.id}) → HTTP ${install.status}: ${(await install.text().catch(() => "")).slice(0, 200)}`);
                }
            }
        ...
        const syncAgents = async (baseUrl, botId, agents) => {
            if (!agents)
                return; // tura bez peerów albo driver bez uprawnienia — nie ma co montować
            const signature = JSON.stringify(agents);
            if (syncedAgents.get(botId) === signature)
                return; // ten sam spawn — zero HTTP
            ...
            const res = await fetch(`${baseUrl}/api/bots/${encodeURIComponent(botId)}/mcp/${encodeURIComponent(ENGINE_AGENTS_MCP)}`, {
                method: "PUT",
                headers: { "content-type": "application/json" },
            ...
            if (!res.ok) {
                throw new Error(`engine PUT /api/bots/${botId}/mcp/${ENGINE_AGENTS_MCP} → HTTP ${res.status}: ${(await res.text().catch(() => "")).slice(0, 200)}`);
            }
        ...
        const seenCount = (threadId) => {
            ...
            for (const line of log.split("\n")) {
                ...
                if (entry.source === "slafy.sync")
                    seen = Number(entry.msg?.seen) || 0;
                else if (entry.source === "slafy.chat" && entry.dir === "out")
                    seen++;
                else if (entry.source === "slafy.chat" && entry.dir === "in" && String(entry.msg?.reply ?? "").trim())
                    seen++;
            }
            return seen;
        };
        const syncThread = async (baseUrl, botId, threadId) => {
            ...
            if (!Array.isArray(history) || seen > history.length)
                return;
            const turnId = newId();
            let emitted = 0;
            ...
            if (emitted)
                emit({ ...base(threadId, turnId), type: "turn.completed", ok: true, cost: null });
        };
        const attachSync = async () => {
            ...
            for (const bot of Array.isArray(bots) ? bots : []) {
                ...
                if (threadId)
                    await syncThread(baseUrl, bot.id, threadId);
            }
        };
        // ── F4: zgody ──────────────────────────────────────────────────────────
        ...
        const requestTurn = new Map(); // requestId → turnId, na którym padła prośba
        ...
        const respondToRequest = async (threadId, requestId, decision) => {
            ...
            const res = await fetch(`${engineBaseUrl()}/api/bots/${encodeURIComponent(botId)}/approvals/${encodeURIComponent(requestId)}`, {
                method: "POST",
                headers: { "content-type": "application/json" },
        ...
        const sendTurn = async (turn) => {
            ...
            await syncConnectors(baseUrl).catch((e) => appendNative(threadId, { dir: "out", source: "slafy.mcp", msg: { error: String(e) } }));
            ...
            await syncAgents(baseUrl, botId, turn.integrations?.agents).catch((e) => appendNative(threadId, { dir: "out", source: "slafy.agents", msg: { error: String(e) } }));
            const turnId = newId();
            ...
            void (async () => {
                ...
                try {
                    ...
                    for await (const { name, payload } of sseFrames(res.body, abort.signal)) {
                        switch (name) {
                            ...
                            case "approval_resolved": {
                                ...
                                const requestId = String(payload.request_id ?? "");
                                if (!requestId || !requestTurn.delete(requestId))
                                    break;
                                emit({
                                    ...base(threadId, turnId),
                                    requestId,
                    ...
                    if (!done && !failure)
                        failure = "engine stream ended before the turn completed";
                }
                catch (e) {
                    failure = e.name === "AbortError" ? null : e.message;
                }
                finally {
                ...
                if (failure)
                    emit({ ...base(threadId, turnId), type: "runtime.error", message: failure });
                emit({
                    ...base(threadId, turnId),
                    type: "turn.completed",
                    ok: done && !failure,
