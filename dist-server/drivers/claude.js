...
const canonicalModel = (model) => {
    ...
    if (model === "opus" || model.startsWith("claude-opus-"))
        return "claude-opus-5";
    if (model === "haiku" || model.startsWith("claude-haiku-"))
        ...
        close() {
            for (const p of [...pending.values()]) {
                if (p.ask.kind === "question")
                    p.finish("answer", "MultiBot: the turn is ending — wrap up.", "shutdown");
                else
                    p.finish("deny", "MultiBot: the turn ended", "shutdown");
            }
            try {
                server.close();
            }
    ...
    async create(input) {
        ...
        const sendTurn = async (turn) => {
            ...
            if (!worker || worker.child.stdin?.destroyed) {
                ...
                const env = { ...process.env, PATH: augmentedPath(), NPM_CONFIG_LOGLEVEL: "error", MCP_TIMEOUT: "30000" };
